...
编译过程：
在使用configure生成Makefile时，添加```CFLAGS=-static –enable-static LDFLAGS=-static –disable-shed -prefix=指定路径```参数，从而生成用于静态编译的Makefile文件。
之后是正常的```make```与```make install```用于编译安装ghostscript，由于在configure阶段设置了```prefix```参数，于是安装的位置会定位所指定的路径，便于移植。
由于目的是移植与测试，所以将 指定路径/share/man目录进行了删除，其所存文件为用于man手册的帮助文档
...
首先在虚拟机中添加usb控制器用于连接打印机，并连接PC与打印机。在/data/user/ghostscript/bin目录下执行```gs -q -dBATCH -dSAFER -dQUIET -dNOPAUSE -sPAPERSIZE=a4 -r600x600 -sDEVICE=pbmraw -sOutputFile=test_1.pbm chess.ps ```命令，将测试文件chess.pdf进行格式转换并命令为test_1.pbm，输出在当前目录中，执行完毕后在当前目录下确实出现了test__1.phm文件，ghostscript运行成功。
接下来进入foo2zjs的目录下执行```./foo2zjs -z3 -p9 -r600x600 ../gs-install/bin/test_1.pbm > /dev/usb/lp0```命令，但是执行后出现错误，错误信息提示无lp0设备文件。
